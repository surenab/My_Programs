source /remote/am04home1/arpih/bin/from_suren/chcell_maker/get_ip_op_pin.tcl
source /remote/am04home1/arpih/bin/from_suren/chcell_maker/chcell_dialog.tcl
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#////////////////////////VIA sizes
set via12_size_x 0.032
set via12_size_y 0.032
set via23_size_x 0.032
set via23_size_y 0.032
set via34_size_x 0.032
set via34_size_y 0.032
set via45_size_x 0.032
set via45_size_y 0.032
#//////////////////////////Extansion nets
...
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
set outl_layer [list "OUTL" "drawing"]
...
set v12_layer [ list "VA12" "drawing"]
set v23_layer [ list "VA23" "drawing"]
set v34_layer [ list "VA34" "drawing"]
set v45_layer [ list "VA45" "drawing"]
#//////////////////////////////seting edge cell names and lib name//////////////////////////////////////////////////////////////////////
set edge_libname "chcells_lay_hc"
...
proc get_flatten_shapes {cname lname} {
	...
	if {[catch {dm::copyCells [dm::findCell $cname -libName $lname] -libDestName $lname  -cellDestName for_chcell} resu]} {
		puts "Couldn't copy cell"
		 return 0;
	}
	puts "Cell is copied"
	#puts "point 0"
...
proc get_x_y_edge {} {
	...
	if {[catch {set cell_dell [oa::DesignOpen $edge_libname $edge_cellname_h_t layout readOnly]} resedg1]} {
		set errmsg "ERROR::I can not find  ''$edge_cellname_h_t'' cell in  ''$edge_libname''  lib.Please check  $edge_cellname_h_t cell."
        	puts $errmsg 
		errorMessage $errmsg
		 return 0;
	} else {
		...
		if {$dx_h == ""} {
			set dx_h 0
			puts "ERROR::I can not found $outl_layer layer  in $edge_cellname_h_t cell"
		}
		if {$dy_h == ""} {set dy_h 0}
		if {[catch {set cell_dell [oa::DesignOpen $edge_libname $edge_cellname_v layout readOnly]} resv]} {
			set errmsg "ERROR::I can not find ''$edge_cellname_v''cell in ''$edge_libname'' lib.Please check  $edge_cellname_v cell."
        		puts $errmsg 
			errorMessage $errmsg
			 return 0;
		} else {
			...
			if {$dx_v == ""} {
				set dx_v 0 
				puts "ERROR::I can not found $outl_layer layer in $edge_cellname_v cell"
			}
			if {$dy_v == ""} {set dy_v 0}
...
proc level0 {design lname cname count orient} {
	...
	if {$orient == "h"} {
		if {[catch {set chcell_length [expr $count*$dx]} resx]} {
			puts "ERROR::I can not find '' $outl_layer '' layer.Please check that layer in $cname cell."
			set errmsg "ERROR::I can not find '' $outl_layer '' layer.Please check that layer in $cname cell."
        		errorMessage $errmsg
			 return 0;
		}
		set chcell_he $dy
		...
		if {[catch {set chcell_he [expr $count*$dy]} resy]} {
			puts "ERROR::I can not find '' $outl_layer '' layer.Please check that layer in $cname cell."
			set errmsg "ERROR::I can not find '' $outl_layer '' layer.Please check that layer in $cname cell."
        		errorMessage $errmsg
			 return 0;
...
proc level1 {edd lname cname count orient metal1} {
	...
	#set lay_ports1 [db::getShapes -of $cell_dell -lpp {LVSM1 drawing }]
...
proc level2 {edd lname cname count orient metal2 metal1} {
	...
	set lay_ports2 [db::getShapes -of $cell_dell -lpp {LVSM2 drawing }]
...
proc level3 {edd lname cname count orient metal3 metal2} {
	...
	set lay_ports3 [db::getShapes -of $cell_dell -lpp {LVSM3 drawing }]
...
proc level4 {edd lname cname count orient metal4 metal3} {
	...
	set lay_ports4 [db::getShapes -of $cell_dell -lpp {LVSM4 drawing }]
...
proc port_without {netlist edd lname cname count chcell_cellname orient} {
	...
	for {set i 0} {$i<[llength $port_info]} {incr i} {
		...
		if {([lsearch $ports_without $pin_name_h]!=-1) || ([lsearch $ports_without $pin_name_h_lower]!=-1)} { 
			...
			if {$orient == "h"} {
				if {$count == 1} {
					le::createLabel $pin_name_h -parent $edd -lpp [list [lindex [lindex $port_info $i] 2] "drawing"] -origin [list $tmpx $tmpy] -height 0.02
				}
				if {$count == 2} {
					le::createLabel $pin_name_h -parent $edd -lpp [list [lindex [lindex $port_info $i] 2] "drawing"] -origin [list $tmpx $tmpy] -height 0.02
					...
					le::createLabel $pin_name_h -parent $edd -lpp [list [lindex [lindex $port_info $i] 2] "drawing"] -origin [list [expr 1*$outl_x+$tmpx] $tmpy] -height 0.02
				}
				if {$count == 3} {
					le::createLabel $pin_name_h -parent $edd -lpp [list [lindex [lindex $port_info $i] 2] "drawing"] -origin [list $tmpx $tmpy] -height 0.02
					if {(($orient == "h")|| ([lsearch $bacar_pins $old_pin_name] != -1))&& ([lsearch $net_dont_p $pin_name_h] == -1)} {set pin_name_h [plus_fin_char_count $pin_name_h $ports_without $count]}
					le::createLabel $pin_name_h -parent $edd -lpp [list [lindex [lindex $port_info $i] 2] "drawing"] -origin [list [expr 1*$outl_x+$tmpx] $tmpy] -height 0.02
					if {(($orient == "h")|| ([lsearch $bacar_pins $old_pin_name] != -1))&& ([lsearch $net_dont_p $pin_name_h] == -1)} {set pin_name_h [plus_fin_char_count $pin_name_h $ports_without $count]}
					le::createLabel $pin_name_h -parent $edd -lpp [list [lindex [lindex $port_info $i] 2] "drawing"] -origin [list [expr 2*$outl_x+$tmpx] $tmpy] -height 0.02
				}
				if {$count == 4} {
					le::createLabel $pin_name_h -parent $edd -lpp [list [lindex [lindex $port_info $i] 2] "drawing"] -origin [list $tmpx $tmpy] -height 0.02
					if {(($orient == "h")|| ([lsearch $bacar_pins $old_pin_name] != -1))&& ([lsearch $net_dont_p $pin_name_h] == -1)} {set pin_name_h [plus_fin_char_count $pin_name_h $ports_without $count]}
					...
					le::createLabel $pin_name_h -parent $edd -lpp [list [lindex [lindex $port_info $i] 2] "drawing"] -origin [list [expr 2*$outl_x+$tmpx] $tmpy] -height 0.02
					if {(($orient == "h")|| ([lsearch $bacar_pins $old_pin_name] != -1))&& ([lsearch $net_dont_p $pin_name_h] == -1)} {set pin_name_h [plus_fin_char_count $pin_name_h $ports_without $count]}
					le::createLabel $pin_name_h -parent $edd -lpp [list [lindex [lindex $port_info $i] 2] "drawing"] -origin [list [expr 3*$outl_x+$tmpx] $tmpy] -height 0.02
				}
			} else {
				if {$count == 1} {
					le::createLabel $pin_name_h -parent $edd -lpp [list [lindex [lindex $port_info $i] 2] "drawing"] -origin [list $tmpx $tmpy] -height 0.02
				...
				if {$count == 2} {
					...
					le::createLabel $pin_name_h -parent $edd -lpp [list [lindex [lindex $port_info $i] 2] "drawing"] -origin [list $tmpx [expr 1*$outl_1+$tmpy]] -height 0.02
				}
				if {$count == 3} {
					le::createLabel $pin_name_h -parent $edd -lpp [list [lindex [lindex $port_info $i] 2] "drawing"] -origin [list $tmpx $tmpy] -height 0.02
					if {([lsearch $bacar_pins $old_pin_name] != -1)} {set pin_name_h [plus_fin_char_count $pin_name_h $ports_without $count]}
					le::createLabel $pin_name_h -parent $edd -lpp [list [lindex [lindex $port_info $i] 2] "drawing"] -origin [list $tmpx [expr 1*$outl_y+$tmpy]] -height 0.02
					if {([lsearch $bacar_pins $old_pin_name] != -1)} {set pin_name_h [plus_fin_char_count $pin_name_h $ports_without $count]}
					le::createLabel $pin_name_h -parent $edd -lpp [list [lindex [lindex $port_info $i] 2] "drawing"] -origin [list $tmpx [expr 2*$outl_y+$tmpy]] -height 0.02
				...
				if {$count == 4} {
					...
					le::createLabel $pin_name_h -parent $edd -lpp [list [lindex [lindex $port_info $i] 2] "drawing"] -origin [list $tmpx [expr 2*$outl_y+$tmpy]] -height 0.02
					if {([lsearch $bacar_pins $old_pin_name] != -1)} {set pin_name_h [plus_fin_char_count $pin_name_h $ports_without $count]}
					le::createLabel $pin_name_h -parent $edd -lpp [list [lindex [lindex $port_info $i] 2] "drawing"] -origin [list $tmpx [expr 3*$outl_y+$tmpy]] -height 0.02
				}
			}
		} else {
...
proc port_ip {netlist edd lname cname count chcell_cellname orient} {
	...
	if {[llength $ports_iph] == 0} { 
		...
		if {[catch {LSV $layer_0_lev} reslsv]} {
			puts "ERROR::I can not find ''LSV'' command in you cdesigner"
			 return 0;
		} 
		LSV $layer_sram
	...
	set port_with_shapes_iph [get_my_nets $port_with_shapes_iph [list "M3" "M1" ]]
...
#//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////		
	...
	for {set i 0} {$i<[llength $port_with_shapes_ipv]} {incr i} {
		...
		if {$pin_layer_h == ""} {
			puts "ERROR::I can not find type of layer for [lindex $port_with_shapes_iph $i] port.I set default value M1 layer "
			set pin_layer_h "M1"
		}
		if {[catch {set x_origin_h [expr [lindex [lindex $pin_cod_h 1] 0]-[lindex [lindex $pin_cod_h 0] 0]]} resyh]} {
			puts "ERROR::I can not find Coordinates for [lindex $port_with_shapes_ipv $i] port"
			set x_origin_h -1
		}
		if {[catch {set x_origin_h [expr $x_origin_h/2+[lindex [lindex $pin_cod_h 0] 0]]} resyh2]} {
			puts "ERROR::I can not find Coordinates for [lindex $port_with_shapes_ipv $i] port"
		}
		if {[catch {set tmp_my_y [lindex [lindex $pin_cod_h 1] 1]} rexh]} {
			puts "ERROR::I can not find Coordinates for [lindex $port_with_shapes_ipv $i] port"
			set tmp_my_y -1
		}
		...
		if {[catch {set y_origin_h [expr [lindex [lindex $pin_cod_h 1] 1]-[lindex [lindex $pin_cod_h 0] 1]]} rexh]} {
			puts "ERROR::I can not find Coordinates for [lindex $port_with_shapes_ipv $i] port"
			set y_origin_h -1
		}
...
#//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	for {set i 0} {$i<[llength $port_with_shapes_iph]} {incr i} {
		...
		if {[catch {set y_origin_h [expr [lindex [lindex $pin_cod_h 1] 1]-[lindex [lindex $pin_cod_h 0] 1]]} resyh]} {
			puts "ERROR::I can not find Coordinates for [lindex $port_with_shapes_iph $i] port"
			set y_origin_h -1
		}
		if {[catch {set y_origin_h [expr $y_origin_h/2+[lindex [lindex $pin_cod_h 0] 1]]} resyh2]} {
			puts "ERROR::I can not find Coordinates for [lindex $port_with_shapes_iph $i] port"
		}
		if {[catch {set x_origin_h [expr [lindex [lindex $pin_cod_h 1] -0]-[lindex [lindex $pin_cod_h 0] 0]]} rexh]} {
			puts "ERROR::I can not find Coordinates for [lindex $port_with_shapes_iph $i] port"
			set x_origin_h -1
		}
...
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
source /remote/am04home1/arpih/bin/from_suren/chcell_maker/exceptions_array.tcl	
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
proc main {netlist chcell_cellname chcell_libname or} {
	...
	if {($res_char == 0) && ($res_red == 0)} {
		...
		if {$err1!=-1} {
			...
			puts "\n//////////////////////////////Stsrting level 0//////////////////////////////////////////////////////////"
		       if {[catch {level0 $desoa $lib_name $cell_name $count $orient} res0]} {
		              puts "I can ot create level 0"
		              set errmsg "I can not create level 0.Please see in console and solve problem"
        	              errorMessage $errmsg
		              return 0;
		       }
			puts "level 0 is complate"
			#level1 $desoa $lib_name $cell_name $count $orient [lindex $metal_shapes 0]
			if {[catch {level1 $desoa $lib_name $cell_name $count $orient [lindex $metal_shapes 0]} res1]} {
				puts "ERROR::I can not create level 1"
				set errmsg "ERROR::I can not create level 1.Please see in console and solve problem"
        			errorMessage $errmsg
				 return 0; 
			}
			puts "level 1 is complate"
			#level2 $desoa $lib_name $cell_name $count $orient [lindex $metal_shapes 1] [lindex $metal_shapes 0]
			if {[catch {level2 $desoa $lib_name $cell_name $count $orient [lindex $metal_shapes 1] [lindex $metal_shapes 0]} res2]} {
				puts "ERROR::I can ot create level 2"
				set errmsg "ERROR::I can not create level 2.Please see in console and solve problem"
        			errorMessage $errmsg
				 return 0; 
			}
			puts "level 2 is complate"
			#level3 $desoa $lib_name $cell_name $count $orient [lindex $metal_shapes 2] [lindex $metal_shapes 1]
			if {[catch {level3 $desoa $lib_name $cell_name $count $orient [lindex $metal_shapes 2] [lindex $metal_shapes 1]} res3]} {
				puts "ERROR::I can ot create level 3"
				set errmsg "ERROR::I can not create level 3.Please see in console and solve problem"
        			errorMessage $errmsg
				 return 0; 
			}
			puts "level 3 is complate"
			#level4 $desoa $lib_name $cell_name $count $orient [lindex $metal_shapes 3] [lindex $metal_shapes 2]
			if {[catch {level4 $desoa $lib_name $cell_name $count $orient [lindex $metal_shapes 3] [lindex $metal_shapes 2]} res4]} {
				puts "ERROR::I can ot create level 4"
				set errmsg "ERROR::I can not create level 4.Please see in console and solve problem"
        			errorMessage $errmsg
				 return 0; 
			}
			puts "level 4 is complate"
			#level5 $desoa $lib_name $cell_name $count $orient
			if {[catch {level5 $desoa $lib_name $cell_name $count $orient} res5]} {
				puts "ERROR::I can ot create level 5"
				set errmsg "ERROR::I can not create level 5.Please see in console and solve problem"
        			errorMessage $errmsg
				 return 0; 
			}
			...
			if {[catch {set p_does_exist [port_without $netlist $desoa $lib_name $cell_name $count $chcell_cellname $orient]} resdop]} {
				puts "ERROR::I can ot create ports without ip op sufix"
				set errmsg "ERROR::I can not create ports without ip op sufix.Please see in console and solve problem"
        			errorMessage $errmsg
				 return 0; 
			}
			puts "\n\n\n Starting create ports with IP OP SUFIX\n\n\n"
			set ip_does_exist [port_ip $netlist $desoa $lib_name $cell_name $count $chcell_cellname  $orient]
			if {[catch {set ip_does_exist [port_ip $netlist $desoa $lib_name $cell_name $count $chcell_cellname  $orient]} resdop]} {
				puts "ERROR::I can ot create ports with ip op sufix"
				set errmsg "ERROR::I can not create ports with ip op sufix.Please see in console and solve problem"
        			errorMessage $errmsg
				 return 0; 
			}
