set char_edge_lib 	"TSMC_16FFV0D2"
set char_edge_left_y  	"16FF_d0734_v0d2_row_edge_sep_Vss_x2_l"
set char_edge_right_y 	"16FF_d0734_v0d2_row_edge_sep_Vss_x2_r"
set char_edge_00	"16FF_d0734_v0d2_corner_dummy_sep_wells_x1_l"
set char_edge_01	"16FF_d0734_v0d2_corner_dummy_sep_wells_x1_l"
set char_edge_10	"16FF_d0734_v0d2_corner_dummy_sep_wells_x1_r"
set char_edge_11	"16FF_d0734_v0d2_corner_dummy_sep_wells_x1_r"
set char_edge_top   	"16FF_d0734_v0d2_sep_wells_strap_edge_x2"
set char_edge_bot   	"16FF_d0734_v0d2_sep_wells_strap_edge_x2"
...
proc exp_level0 {design lname cname rx cx } {
	...
	set ii $rx
	while {$ii} {
		le::createInst -design $design -origin [list 0 [expr ($ii-1)*$dy]] -libName $char_edge_lib -cellName $char_edge_left_y 	-viewName layout -orient "R0" -rows 2 -dy [expr $dy/2]
		set ii [expr $ii -1]
	}
	
	set ii $rx
	while {$ii} {
		le::createInst -design $design -origin [list [expr $const_countc*$dx] [expr ($ii-1)*$dy]] -libName $char_edge_lib -cellName $char_edge_right_y  	-viewName layout -orient "MY" -rows 2 -dy [expr $dy/2]
		set ii [expr $ii -1]
	}
...
proc port_ip_char16 {netlist edd lname cname  chcell_cellname rx cx} {
	...
	set layer_sram 	[list "COD_V" "EXCLVS" "M1 drawing5" "SRMD10" "COD_H" "OD_DA" "M1 drawing6" "PO1SRAM" "VIA0 drawing1" "VIA0 drawing2" "M2 drawing6" "M2 drawing5" "ARY drawing" "FB_SRM drawing1" "VIA0 drawing8"]
...
#//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////		
	#puts "port_with_shapes_ipv = $port_with_shapes_ipv"
		...
		for {set ix 0} {$ix<$cx} {incr ix} {
			...
			if {$ix == 0} {
				...
				set pin_name_h [plus_fin_char_count_char16 $pin_name_h $ports_ipv $cx]
	       			set pin_name_oph [plus_fin_char_count_char16 $pin_name_oph $ports_ipv $cx]
	       			#puts "created 2 pin names" 
	       			set ip_cood [list [expr ($cx-$ix)*$dx_cell+$x_origin_h] $y_start_h]
	       			set op_cood [list [expr ($cx-$ix)*$dx_cell+$x_origin_h] [expr $dy_cell-0.005]]
	       			set lvs_layer "LVS$pin_layer_h"
	       			set xlvs_layer "XLVS$pin_layer_h"
	       			le::createLabel $pin_name_h -parent $edd -lpp [list  $lvs_layer "drawing"] -origin $ip_cood -height 0.02
	       			lappend ex_ipv $pin_name_h
		...
		#puts "x_start_h = $x_start_h"
	       ...
	       for {set ix 0} {$ix<$rx} {incr ix} {
	              if {$ix == 0} {
	        	      ...
	        	      set op_cood [list [expr $rx*$x_origin_h-0.005] $y_origin_h]
	        	      set lvs_layer "LVS$pin_layer_h"
	        	      set xlvs_layer "XLVS$pin_layer_h"
	        	      le::createLabel $pin_name_h -parent $edd -lpp [list  $lvs_layer "drawing"] -origin $ip_cood -height 0.02
	        	      ...
	        	      set pin_name_h [plus_fin_char $pin_name_h]
	        	      set pin_name_oph [plus_fin_char $pin_name_oph]
	        	      set ip_cood [list $x_start_h [expr ($rx-$ix)*$dy_cell+$y_origin_h]]
	        	      set op_cood [list [expr $rx*$x_origin_h-0.005] [expr ($rx-$ix)*$dy_cell+$y_origin_h]]
	        	      set lvs_layer "LVS$pin_layer_h"
	        	      set xlvs_layer "XLVS$pin_layer_h"
	        	      le::createLabel $pin_name_h -parent $edd -lpp [list  $lvs_layer "drawing"] -origin $ip_cood -height 0.02
	        	      lappend ex_iph $pin_name_h
...
proc main_exp_char {netlist chcell_cellname chcell_libname or} {
	...
	if {$rxcx == 1} {
		if {[string is integer [string range $chcell_cellname end-1 end]] == 1} {
		...
		if {[string is integer [string range $chcell_cellname end-4 end-3]] == 1} {
			set rx  [string range $chcell_cellname end-4 end-3]
			set rx [expr $rx*1]
		} else {
			set rx  [string range $chcell_cellname end-3 end-3]
			set rx [expr $rx*1]
		}
		set cx [expr $cx/4]
		set rx [expr $rx/4]
