...
source ~/.cshrc
module load msip_shell_calex/2014.10
which icv
...
if !(-d $WorkPath/drc_lvs/lvs_$1/) then
	echo "\033[1;29mDoesn't exist lvs_$1/ Directory,I create gds Directory now\033[0m"
...
if (-f $WorkPath/drc_lvs/cell_list) then
	foreach cellName (`cat $WorkPath/drc_lvs/cell_list`) 
		printf "\ncell		$cellName		$cellName		L">>cover_lvs_$1.cnt
	end
else
	echo "\033[1;29m\nDoes not exist <cell_list> file \nPlease write cell_list and become run script\n\033[0m"
	exit
endif	
if (-d $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs_old) then
	echo "\033[1;29mRemoving calibre_lvs_old directory\033[0m"
	rm -rf $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs_old 
endif
if (-d $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs) then
	echo "\033[1;29mCopy icv_lvs in calibre_lvs_old directory\033[0m"
	mv -f $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs_old
endif
...
echo "\033[1;33mYou use CALIBRE verification for $cellName cell\033[0m"
...
foreach cellName (`cat $WorkPath/drc_lvs/cell_list`) 
	if (-f $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs/$cellName/calibre_lvs.log) then
		echo "\033[1;34mFor $cellName\033[0m"
		setenv vuecell $cellName
		cat $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs/$cellName/calibre_lvs.log | grep "LVS completed."
		if (-f $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs/$cellName/LVS.lvs.ext) then
			#cat $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs/$cellName/LVS.lvs.ext 
			grep 'Short circuit' $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs/$cellName/LVS.lvs.ext ; grep 'Open circuit' $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs/$cellName/LVS.lvs.ext ; grep 'Unattached label:' $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs/$cellName/LVS.lvs.ext ; grep conflict $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs/$cellName/LVS.lvs.ext 
			grep "TOTAL ERC RuleCheck Results Generated:" $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs/$cellName/erc_summary.report
			nedit $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs/$cellName/LVS*
		else
			echo "\033[1;34mDoesn't exist LVS.lvs.ext file for $cellName\033[0m"
			echo "\033[1;34mCheck LOG files in $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs/$cellName directory\033[0m"
			
		endif
		 
	else
		echo "\033[1;34mDoesn't exist LVS.lvs file for $cellName\033[0m"
		echo "\033[1;34mCheck LOG files in $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs/$cellName directory\033[0m"
		cat $WorkPath/drc_lvs/lvs_$1/cells/calibre_lvs/$cellName/*log | grep error
	endif
end
